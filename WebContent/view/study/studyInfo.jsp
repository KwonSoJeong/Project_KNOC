				...
				<div class="panel panel-default clearfix fa-">
					...
					<div class="content-container clearfix">
						...
						<div class="content-tags">
								<button type="submit" onclick="javascript: f.action='<%=request.getContextPath() %>/study/studyChangeProcess';" style="border: none;" class='<c:if test="${s.process==1 }">list-group-item-text item-tag label label-info'> <i class="fa fa-comments"></i>모집중</c:if>
					...
					<ul class="list-group">
							...
							<form action="/content/update/1570494" method="post" data-id="1570494" class="note-update-form">
								...
								<div class="content-body panel-body pull-left">
									<div class="avatar clearfix avatar-medium">
										<c:choose>
										<c:when test="${commentProfileList[status.index]!=null}"><span class="avatar-photo"><img src="<%=request.getContextPath() %>/profile/${commentProfileList[status.index] }" /></span></c:when>
						...
						<li class="list-group-item note-form clearfix">
							<div class="panel-body">
								...
								<c:choose>
									<c:when test="${memid==null}"><a href="<%=request.getContextPath()%>/member/login" class="link">로그인</a>을 하시면 댓글을 등록할 수 있습니다.</c:when>
									<c:otherwise>
